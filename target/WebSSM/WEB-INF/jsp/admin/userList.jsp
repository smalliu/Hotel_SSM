...
<script>
    ...
    function delUserById(userId) {
        $.ajax(
            {
                url: "${ctxPath}/Admin/delUser",
                type: "post",
                data: {id: userId},
                dataType: "json",
                success: function (json, status, xhr) {
                    console.log(json);
                    window.location = "${ctxPath}/Admin/viewUserList";
                }
            }
        );
    }
...
//	console.log( "--------------------------------------"  );
        ...
        //{ps} 生成 6 项数据
        //{1} 只读数据: account (input)
        table += makeInput('用户帐号', 'account', 'readonly',
            json['user']['account']);
        //{2} 可改数据: 名称 (input)
        table += makeInput('用户名称', 'nickName', '',
            json['user']['nickName']);
        //{3} 可改数据: 密码 (input)
        table += makeInput('用户密码', 'password', '',
            json['user']['password']);
        //{4} 可改数据: 性别 (select)
        //makeSelect('性别','sex', 列表项的数组, val )
        table += makeSelect('性别', 'sex', sex_data,
            json['user']['sex']);
        //{5} 可改数据: 角色 (select)
        table += makeSelect('角色', 'roleId', role_data,
            json['user']['roleId']);
        ...
        $.ajax(
            {
                ...
                type: "post",
                data: {
                    id: _id, account: _account, nickName: _nickName,
                    password: _password, sex: _sex, roleId: _roleId
                },
                dataType: "json",
    ...
    layui.use(
        ...
        function () {
            ...
            table.render({
                elem: '#demo'
                , height: 560
                , url: '${ctxPath}/Admin/userList' //这是一个数据接口，拿到json数据，重新绘制表格
                , title: '用户表'
                , page: true //开启分页
                ...
                , cols: [[ //表头
                    ...
                    , {field: 'nickName', title: '名称', width: 120, sort: true, totalRow: true}
                    , {field: 'sex', title: '性别', width: 80, sort: true}
                    , {field: 'no', title: '工号', width: 80, sort: true, totalRow: true}
